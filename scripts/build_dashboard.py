...
(function () {{
  ...
  function render() {{
    ...
    const traces = [], annos = [];
    function seg(y, x0, w, color, grp, hover, legendName) {{
      traces.push({{ type: "bar", orientation: "h", y: [y], x: [w], base: [x0],
        width: H, name: legendName || "", showlegend: !!legendName, meta: grp,
    ...
    // ── 막대 1: 전체 필진 ──
    seg(YS[0], 0, pAuth, GOLD, "s", "전체 필진 · 인철과 학맥 " + A_S + "명", "인철과 학맥 출신");
    seg(YS[0], pAuth, 100 - pAuth, GREY, "o", "전체 필진 · 그 외 " + (TOT_A - A_S) + "명", "그 외");
    ...
    let left = 0;
    [[gCore, GOLD, "핵심 몫 " + gCore + "편", true, "s"],
     [gOnce, GOLD_P, "일회성 " + gOnce + "편", false, "s"],
     [oCore, GREY_D, "핵심 몫 " + oCore + "편", true, "o"],
     [oOnce, GREY, "일회성 " + oOnce + "편", false, "o"]].forEach(([cnt, col, lab, white, grp], i) => {{
      const w = 100 * cnt / TOT_W;
      seg(YS[1], left, w, col, grp, (grp === "s" ? "인철과 학맥 · " : "그 외 · ") + lab);
      if (w >= 9) {{
    ...
    // ── 막대 3: 핵심 필진 ──
    seg(YS[2], 0, pCore, GOLD, "s", "핵심 필진 · 인철과 학맥 " + nCoreS + "명");
    seg(YS[2], pCore, 100 - pCore, GREY, "o", "핵심 필진 · 그 외 " + (nCore - nCoreS) + "명");
  ...
  // ── 호버 연동: 같은 출신(학맥/그 외)을 세 막대에서 함께 강조 ──
  let nTraces = 0, ribbonIdx = -1, hl;
  function setHighlight(grp) {{
    if (hl === grp) return;
    hl = grp;
    const ops = [];
    for (let i = 0; i < nTraces; i++) ops.push(grp == null || gd.data[i].meta === grp ? 1 : 0.25);
    Plotly.restyle(gd, {{ opacity: ops }});
    Plotly.relayout(gd, {{ ["shapes[" + ribbonIdx + "].fillcolor"]:
      grp === "o" ? "rgba(184,145,31,0.04)" : "rgba(184,145,31,0.13)" }});
  }}
  ...
  render();
  gd.on("plotly_hover", ev => setHighlight(ev.points[0].data.meta));
