...
(function () {
  ...
  // 화면 회전 직후 Plotly가 낡은 치수로 그려져 패널을 넘치는 문제 —
  // 회전·리사이즈가 끝난 뒤 전체 차트를 다시 측정한다. 이벤트를 놓치는 경우에
  // 대비해 콘텐츠 영역(main)의 실측 폭 변화도 ResizeObserver로 감시한다.
  var rz;
  function refit() {
  ...
  function queueRefit(ms) { clearTimeout(rz); rz = setTimeout(refit, ms); }
